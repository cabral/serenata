...
- [ ] Session 5: F1 and pair rates. **Built up to its STOP.** `crony flag F1` and `crony base-rate F1` run on `dep:74`: 8,779 pairs in the denominator, 19 candidate pairs, 0 packet-eligible, and 79 would-be hits removed by the exclusion list, more than the 73 hits that remain. The table, per-gate losses and precision are in `$CRONY_DATA_DIR/staged/_reports/f1-base-rate-dep-74.json` for the maintainer; nothing is written into the flag spec until the maintainer approves it. **Revised 2026-09-23 after the maintainer's review of the first table**, which was not approved: the rate is now one overall figure and two bands split at 3,500, precision is a census of all 107 candidates, the limits print beside the rate, and the measure is labelled "co-occurrence with a current officer, counts only" until role dates exist. Approval waits on two things only a person or an account can supply: the maintainer's review of every candidate, and INPI role history.
