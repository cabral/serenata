...
-- values. Structural keys match the normalised model; local join IDs and a
-- single present tender-code block per lot result are additional requirements.
--
-- Scoped to what the rule reads: the publications the population draws from,
-- and for the tender check the lot results in it. A publication contributing
-- no lot outcome contributes to no segment, so its duplicates cannot move a
-- flag. TED publishes such rows — one framework lot result in the measured
-- archive carries its bid count four times — and validating whole tables let
-- data the rule never reads stop every measurement. Eligibility stops short of
-- the buyer country, so a publication cannot escape the check by carrying the
-- duplicate organisations that would exclude it.
CREATE OR REPLACE VIEW eligible AS
...
CREATE OR REPLACE VIEW scoped AS
SELECT DISTINCT source_publication_id FROM eligible;

CREATE OR REPLACE VIEW duplicate_input_keys AS
SELECT 1 FROM procedure WHERE source_publication_id IN (SELECT * FROM scoped)
GROUP BY source_publication_id HAVING count(*) > 1
UNION ALL
SELECT 1 FROM lot WHERE source_publication_id IN (SELECT * FROM scoped)
GROUP BY source_publication_id, ordinal HAVING count(*) > 1
UNION ALL
SELECT 1 FROM lot_result WHERE source_publication_id IN (SELECT * FROM scoped)
GROUP BY source_publication_id, ordinal HAVING count(*) > 1
UNION ALL
SELECT 1 FROM organisation WHERE source_publication_id IN (SELECT * FROM scoped)
GROUP BY source_publication_id, ordinal HAVING count(*) > 1
UNION ALL
SELECT 1 FROM organisation_role
WHERE source_publication_id IN (SELECT * FROM scoped)
...
UNION ALL
SELECT 1 FROM lot
WHERE lot_id IS NOT NULL AND source_publication_id IN (SELECT * FROM scoped)
GROUP BY source_publication_id, lot_id HAVING count(*) > 1
UNION ALL
SELECT 1 FROM organisation
WHERE org_local_id IS NOT NULL AND source_publication_id IN (SELECT * FROM scoped)
...
WHERE statistic_kind = 'received_submissions' AND statistic_code = 'tenders'
  ...
  AND (source_publication_id, lot_result_ordinal) IN (SELECT * FROM eligible)
...
--    v2: 8,159 lot results, 3,435 single bid (42.1%), 3,790 notices.
SELECT count(*)                             AS lot_results,
       count(*) FILTER (WHERE bids = 1)     AS single_bid,
       count(DISTINCT source_publication_id) AS notices
FROM population;

-- 2. How far segments differ. v2: 26 segments of 50 or more, covering
...
-- 3. The flags. v2: 96 flags in 71 notices, 2.23% of the covered
--    population.
