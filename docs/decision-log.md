...
What has been finished, and why it was done the way it was. Moved out of
[`open-work.md`](open-work.md) so that file stays a list of what is open rather
than a growing archive of what is not.

**This is history, not instruction.** Each entry records what was built, the
measurements that shaped it, and the corrections real data forced — the last of
which is usually the useful part, because it says what the plan got wrong.
Anything still open is in `open-work.md`; anything still binding is in an
[ADR](adr/) or in [`CLAUDE.md`](../CLAUDE.md).

Entries keep their original numbering, so a link to
`open-work.md#12-build-the-normalise-stage` and one to
`decision-log.md#12-build-the-normalise-stage` both work.
...
([#15](open-work.md#15-decide-whether-beneficial-ownership-can-be-analysed-at-all)).
